...
class Hypotheses(object):
    ...
    def max(self):
        """ argmax for index of max probability.
        For use with dpd[max(dpd),:] to get full max tuple."""
...
class Coord(object):
    def __init__(self, r=0, theta=0, drawing=False):
        self.r = r
        self.theta = theta
    ...
    def serialize(self):
        ...
        D["r"] = self.r
        D["theta"] = self.theta
        D["drawing"] = 1 if self.drawing else 0
        return D

    def deserialize(self, D, deserialized_lower_layer=None):
        self.r = D["r"]
        self.theta = D["theta"]
        self.drawing = True if D["drawing"] == 1 else False
        return self

    def __repr__(self):
        return str((self.r, self.theta, self.drawing))

    def __eq__(self, other):
        return self.theta == other.theta and self.drawing == other.drawing  # self.r == other.r 
